...
This file is the workflow authority.
...
2. CHANNEL_VEO_PROMPT_RULES.md
   = controls prompt_raw schema, shot_type logic, field discipline, and QC
...
This workflow does not require the assistant to output downstream full generation prompts by default.
...
3. visual_grouping
4. storyboard_draft
5. timing_budget_check
6. optimization_suggestions
7. optimized_shot_plan only after user approval

PHASE B — PROMPT PRODUCTION
8. narration_cue
9. visual_goal
10. viewer_sees
11. layout / shot_type mapping
12. prompt_raw
13. GKStudio / projectbeat JSON
```

This order is mandatory.

Do not start from `prompt_raw`.

Do not start from style.

Do not start from tags.

Do not start from a generic Veo prompt.

Do not start from SRT.
...
It is a mini visual map when no separate visual map file is used.
...
```text
Bắt đầu bằng hình ảnh quyền lực quen thuộc: ngai, đại điện, áo bào và cổng cung. Sau đó điểm nhìn rút xuống các lớp vận hành bên dưới: văn lại, văn thư, đường rời trung tâm, người đọc luật, người đếm thóc. Beat kết thúc bằng cảm giác rằng ngai vàng không tự mình cai trị; quyền lực phải đi qua một bộ máy bên dưới.
```

Weak:

```text
Ngai vàng → hồ sơ → đường → thóc → hệ thống.
```
...
```text
Group | Shot | Visual idea | Layout | Min visual time
```
...
Storyboard Draft must answer:

```text
What would this beat require if every meaningful visual idea had to stand as its own shot?
```
---
...
```text
Min visual time = time to read the image, not time to read the narration.
```
...
merge shots within the same Group
convert user-approved compressed symbolic clusters to SPLIT_SCREEN_2 or SPLIT_SCREEN_3
use SINGLE_SHOT_TRANSITION only if the user approves one clear before/after visual turn
mark a cluster as manual edit note using MONTAGE_EDIT or MONTAGE_END
```
...
Do not treat Optimization Suggestions as final.
...
It must use the same field order as `storyboard_draft`:
...
`narration_cue` is the exact narration fragment the shot serves.

It should be short enough to make meaning clear.
...
```text
Cho người xem thấy mệnh lệnh của triều đình chỉ trở thành hiện thực khi rời khỏi trung tâm và đi qua người xử lý ở địa phương.
```
...
```text
visual_goal = why this shot exists.
```
...
`viewer_sees` translates the meaning into a concrete clip description for the human operator.
...
Họ đang làm gì?
Đạo cụ/cue nào giúp người xem hiểu đúng?
Không gian, ngưỡng cửa, vị trí cơ thể, hoặc quan hệ xã hội nào làm ý nghĩa rõ?
```
...
```text
Ở điểm thu lương thực bên ngoài quan phủ huyện, vài nông dân đặt bao thóc xuống trước một bàn gỗ thấp. Một người đổ thóc vào giỏ đo, thư lại ngồi phía sau bàn và ghi nhận bằng vật dụng hành chính. Nông dân đứng ở tiền cảnh, còn không gian quan phủ nằm sâu hơn phía sau, tạo cảm giác họ đang nộp gánh nặng vào một bộ máy ở xa hơn họ.
```

Weak:

```text
Thể hiện thuế và tầng trung gian.
```

Core rule:

```text
viewer_sees = what the human operator must actually see.
```

---

## 7.4 QA Anchor Context

Before writing `prompt_raw` for each shot, run QA Anchor Context.

This is a mandatory check.

QA Anchor Context verifies that `prompt_raw.context` follows `CHANNEL_VEO_PROMPT_RULES.md` section 4.2.
...
Do not use CHINA_ERA_LOCK.md by default unless the user explicitly assigns a strict era.
```
...
```text
ancient Chinese imperial court assembly, raised court axis where visible sovereignty dominates the historical memory
imperial Chinese administrative hall, central low desk where an official command packet is prepared before leaving the capital
imperial China rural road near an administrative gate, dusty route where a messenger begins carrying a sealed command toward society
ancient Chinese county office threshold, local administrative space where an order must be processed before reaching villagers
grain tax collection point outside an ancient Chinese county office, villagers submitting grain while a clerk measures the burden
```
...
If QA Anchor Context fails, revise the context before writing `prompt_raw`.
...
QA Anchor Context protects prompt_raw.context before technical construction.
```
---

## 7.6 GKStudio / Projectbeat JSON
...
Is min_visual_time preserved as timing intent?
...
Does prompt_raw follow CHANNEL_VEO_PROMPT_RULES.md?
...
```text
If viewer_sees cannot help a human choose a clip, it is not viewer_sees yet.
```
...
```text
Show a clerk and a road.
```

Stronger human field:

```text
Cho thấy command ban đầu còn nhỏ và chính thức, nhưng khi rời trung tâm và đi qua người xử lý, nó bắt đầu trở thành nơi tầng trung gian có thể tạo margin.
```
...
```text
Cho người xem thấy tax không phải khái niệm trừu tượng; nó thành gánh nặng thật khi thóc bị đo, dân làng phải chờ, và người ngồi sau bàn kiểm soát con số.
```
...
In the new workflow, it also functions as the beat-level mini visual map when no separate visual map file exists.
...
```text
Gói lệnh rời capital trong trạng thái nhỏ, sạch, chính thức — trước khi con đường trung gian biến nó thành burden và margin.
```
...
```text
min_visual_time = timing intent for visual readability.
```
...
prompt_raw = vật liệu kỹ thuật theo CHANNEL_VEO_PROMPT_RULES.md
```
...
Does viewer_sees describe place, subject, action, and meaning cue?
...
Is min_visual_time preserved as timing intent rather than final timing?
...
Did I optimize within the same Group first?
Does the optimized shot plan use: Group | Shot | Visual idea | Layout | Min visual time?
Does each optimized shot become one production shot entry?
Am I preserving min_visual_time as timing intent, not final timecode?
...
preserve the active episode visual lock
avoid schema drift
...
Back to current storyboard-first projectbeat workflow:
01_SCRIPT.md
→ PROJECT_STORYBOARD_PROMPT_ROOM
→ Storyboard Draft
→ Timing Budget Check
...
Which beat are we working on next?
```
...
Optimization protects runtime.
Projectbeat gives operators usable shot cards.
